...
function VoicePlayer({ path }) {
  ...
  useEffect(() => {
    if (!path) return;
    studentApi.get(`/chat/voice/${encodeURIComponent(path)}`, { responseType: 'blob' })
      .then(({ data }) => {
  ...
  return (
    <div className={`flex ${isMe ? 'justify-end' : 'justify-start'} animate-fade-in`}>
      <div className={`max-w-[75%] sm:max-w-[65%] flex flex-col items-${isMe ? 'end' : 'start'}`}>
        ...
        >
          {msg.voiceNotePath ? (
            <VoicePlayer path={msg.voiceNotePath} />
          ) : (
...
export default function StudentChat() {
  ...
  useEffect(() => {
    studentApi.get('/chat/rooms').then(({ data }) => setRooms(data.rooms || [])).catch(() => {}).finally(() => setLoadingRooms(false));
  }, []);
  ...
  useEffect(() => {
    const rid = roomIdParam ? parseInt(roomIdParam, 10) : null;
    if (!rid) return;
    if (rooms.length) setSelectedRoom(rooms.find((r) => r.id === rid) || { id: rid, companyName: 'Chat' });
  }, [roomIdParam, rooms]);
  ...
  useEffect(() => {
    ...
    studentApi.get(`/chat/rooms/${selectedRoomId}/messages`).then(({ data }) => {
  ...
  const sendText = async () => {
    ...
    setSending(true);
    try {
      await studentApi.post(`/chat/rooms/${selectedRoomId}/messages`, { content: text });
      setInput('');
      const { data } = await studentApi.get(`/chat/rooms/${selectedRoomId}/messages`);
      setMessages(data.messages || []);
      studentApi.get('/chat/rooms').then(({ data: d }) => setRooms(d.rooms || [])).catch(() => {});
    } catch (e) {
  ...
  const sendVoice = async () => {
    ...
    try {
      await studentApi.post(`/chat/rooms/${selectedRoomId}/voice`, form, { headers: { 'Content-Type': 'multipart/form-data' } });
      setVoiceFile(null);
      const { data } = await studentApi.get(`/chat/rooms/${selectedRoomId}/messages`);
      setMessages(data.messages || []);
      studentApi.get('/chat/rooms').then(({ data: d }) => setRooms(d.rooms || [])).catch(() => {});
      ...
      <div className="flex flex-1 min-h-0">
        ...
        <div className="w-80 border-r border-slate-200 flex flex-col bg-slate-50/50 shrink-0">
          <div className="p-4 border-b border-slate-200 bg-white">
            ...
            <p className="text-xs text-slate-500 mt-0.5">Chat with applicants & placement team</p>
          </div>
          <div className="flex-1 overflow-y-auto">
            {loadingRooms ? (
              ...
              <div className="p-6 text-center text-slate-500 text-sm">Apply to a drive to see company chats here.</div>
            ) : (
              rooms.map((r) => (
                <button
                  key={r.id}
          ...
          {!activeRoom ? (
            <div className="flex-1 flex items-center justify-center text-slate-500">
              <div className="text-center">
                ...
                <p className="text-sm mt-1">Apply to a placement drive to join its chat</p>
              </div>
            </div>
          ) : (
            <>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-3">
                {loadingMessages ? (
                  ...
                  messages.map((msg) => (
                    <MessageBubble key={msg.id} msg={msg} isMe={msg.senderType === 'STUDENT'} />
                  ))
                )}
